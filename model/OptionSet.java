...
public class OptionSet implements Serializable{
	...
	// Purpose of this method is to find Option, parse it's data, 
	// and return Name, This method is for Choice Interface.
	protected String findOpSelectionName(String name) {
		for(int i=0; i < opt.size(); i++) {
			System.out.println(getOpt(i).getName());
			if(getOpt(i).getName().matches(name)) {
				return getOpt(i).getName();
			}
		}
		return null;	
	}
	protected Double findOpSelectionPrice(String name){
		for(int i=0; i < opt.size(); i++) {
			System.out.println(getOpt(i).getName());
			if(getOpt(i).getName().matches(name)) {
				return getOpt(i).getPrice();
			}
		}
		return null;
	}
	...
	// Add
		protected void addOpt(String splitLine[], int numOpt) throws FixProblems {
			for(int i=0; i < numOpt; i++) {
				try {
					this.opt.add(new Option(splitLine[i*2+1], Double.parseDouble(splitLine[i*2+2])));
